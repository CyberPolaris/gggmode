...
- **`$in`**：软性要求——tag 没检出只是少一分（精确命中要求全中，模糊命中缺了扣分）
- **`$must`**：硬性要求——列出的 tag **缺任何一个，整条规则直接否决**（精确和模糊都不参与）；
  全部在场时正常计入 hit/total

用途：多条规则共用一批 tag 时（如多种人脸窗口都含"人脸验证窗口"），给每条规则指定一个
它独有的必要 tag 作锚点，共用 tag 凑齐也不会冒认——只有锚点同时出现才认这条规则。

**基本写法**——在规则对象里与 `$in`/`$nin` 平级放 `$must` 块，并把锚点 tag 从 `$in`
挪进去（不要两边重复列，重复会在 hit/total 里计两次）：

```jsonc
"普通脸": {
  "$must": { "tags": ["腾讯健康系统_人脸验证标记"] },   // 锚点：缺它整条规则不成立
  "$in": {
    "tags": [
      "腾讯健康系统_人脸验证窗口",
      "腾讯健康系统_开始验证",
  ...
  "$nin": { "tags": ["腾讯健康系统_人脸倒计时"] }
}
```

**`$or` 规则里加锚点**——某个分支需要多个操作符时，用 `$and` 把分支包起来：

```jsonc
"设备脸": {
  "$or": {
    "$and1": {                                          // 分支 1 带自己的锚点
      "$must": { "tags": ["和平精英_人脸识别验证窗口"] },
      "$in":   { "tags": ["和平精英_人脸识别验证窗口内文字", "..."] }
    },
    "$in2": { "tags": ["和平精英_腾讯游戏身份验证", "..."] }
  }
}
```

锚点对所有分支都适用时，直接放在 `$or` 外面与它平级即可。

**怎么选锚点 tag**（两个原则）：

1. **独占性**：用 `TagOwners()`（Python 版 `tag_owners(rules)`）列出每个 tag 被哪些规则
   拥有，只属于一条规则的才适合当它的锚点；共用 tag 当锚点起不到区分作用
2. **检测稳定性**：`$must` 是必要条件，检测器哪怕偶尔漏检这个 tag，整条规则就整帧不成立——
   选画面上大而清晰、检出率高的 tag

若几条规则的画面确实没有独有元素（一个独占 tag 都找不出来），锚点无能为力，
需要从检测模型侧增加新 tag。改完规则后跑一遍现有用例，确认没有误伤原本该命中的画面。
...
1. **规则加 `$must` 锚点**：让每条规则至少有一个独占的必要 tag（定义与写法见第三节）
